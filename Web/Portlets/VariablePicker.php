...
/**
 * Variable picker control.
 *
 * @author Marcin Haba <marcin.haba@bacula.pl>
 * @category Control
 */
class VariablePicker extends Portlets
{
	private const ROOT_HTML_ELEMENT_ID = 'RootHTMLElementId';

	public function loadVariables($sender, $param)
	{
		$variable_config = $this->getModule('variable_config');
		$variables = $variable_config->getConfig();
		$var_config = array_values($variables);
		$cb = $this->getPage()->getCallbackClient();
		$cb->callClientFunction(
			'oVariablePicker.load_variables_cb',
			[$var_config]
		);
	}

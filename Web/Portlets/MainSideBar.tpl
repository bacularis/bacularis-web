...
<nav class="w3-sidebar w3-animate-left w3-margin-bottom" style="z-index: 4; width:250px;" id="sidebar">
	...
	<div class="w3-bar-block" style="margin-bottom: 45px;">
		...
		<a href="<%=$this->Service->constructUrl('NetworkDiagram')%>" class="w3-bar-item w3-button w3-padding<%=$this->Service->getRequestedPagePath() == 'NetworkDiagram' ? ' w3-blue': ''%><%=$this->getModule('users')->isPageAllowed($this->User, 'NetworkDiagram') ? '' : ' hide'%>"><i class="fa fa-network-wired fa-fw"></i>  <%[ Network Diagram ]%></a>

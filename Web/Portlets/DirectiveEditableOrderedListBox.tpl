<div class="directive_field w3-row<%=!$this->display_directive ? ' hide' : '';%>">
	...
	<div class="w3-col w3-threequarter directive_value">
		...
		<i class="fa fa-trash-alt remove_btn<%=!$this->ShowRemoveButton ? ' hide' : ''%>" onclick="oDirectiveOrderedListBox.clear_selection('<%=$this->DirectiveCombo->ClientID%>', '<%=$this->DirectiveHidden->ClientID%>');" alt="<%[ Remove directive ]%>" title="<%[ Remove directive ]%>"></i>

<div id="<%=$this->ClientID%>bulk_actions_modal" class="w3-modal" style="display: none">
	<div class="w3-modal-content w3-card-4 w3-animate-zoom" style="width: 830px">
		...
		<div class="w3-margin-left w3-margin-right" style="max-height: 400px; overflow-x: auto;">
			<div class="w3-code">
				<pre><span id="<%=$this->ClientID%>bulk_actions_output"></span></pre>
			</div>
		</div>
		<footer class="w3-container w3-center w3-border-top">
			...
			<button id="<%=$this->ClientID%>bulk_actions_refresh_page" type="button" class="w3-button w3-section w3-green" style="display: none" onclick="oBulkActionsModal.refresh_page();"><i class="fas fa-redo-alt"></i> &nbsp;<%[ Refresh page ]%></button>
			<i id="<%=$this->ClientID%>bulk_actions_loader" class="fa fa-sync w3-spin w3-margin-left"></i>
		</footer>
	</div>
</div>
<div id="<%=$this->ClientID%>bulk_actions_validation_error_modal" class="w3-modal" style="display: none">
...
var oBulkActionsModal = {
	ids: {
		loader: '<%=$this->ClientID%>bulk_actions_loader',
		output: '<%=$this->ClientID%>bulk_actions_output',
		modal: '<%=$this->ClientID%>bulk_actions_modal',
		validation_err_text: '<%=$this->ClientID%>bulk_actions_validation_error_txt',
		validation_err_modal: '<%=$this->ClientID%>bulk_actions_validation_error_modal',
		refresh_page: '<%=$this->ClientID%>bulk_actions_refresh_page'
	},
	...
	show_error: function(show) {
		const err_modal = document.getElementById(this.ids.validation_err_modal);
		err_modal.style.display = show ? 'block' : '';
	},
	refresh_page: function() {
		document.location.reload();
	},
	show_refresh_btn: function(show) {
		const refresh_page = document.getElementById(this.ids.refresh_page);
		refresh_page.style.display = show ? 'block' : '';
	}
};
</script>
